...
function [coverage] = recomputeCoverage(statistics, outputCoords, outputScales, outputFrames, coverage, X, startX, dx, dy,...
...
%         end 
    ...
    outputScales = outputScales(indEx, :);
    outputFrames = outputFrames(indEx, :);
    ...
    outputScales = outputScales(idx, :);
    outputFrames = outputFrames(idx, :);
    ...
    parfor i = 1:lenF  % for each image
        ...
        scales = outputScales(indsI, :);
        frames = outputFrames(indsI, :);
        
        firstColumn = els(:, 1);
        [~, inddds] = sort(firstColumn);
        els = els(inddds, :);
        scales = outputScales(inddds, :);
        frames = outputFrames(inddds, :);
        ...
        if lenEls > 0   % for some images there are no detected elements   
            ...
            for j = 1:lenEls
                ...
                centre = double([els(j, 3), els(j, 4)]);
                frame = reshape(frames(j,:), [3,3]);
                curScale = double(scales(j, 3));
                vectX = frame(1:2,2)' * dx * curScale;
                vectY = frame(1:2,3)' * dy * curScale;
