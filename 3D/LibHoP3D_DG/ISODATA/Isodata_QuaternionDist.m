...
function [Z, sampleLables] = Isodata_QuaternionDist(X, frequencies, kInit, nMin, iterations, sigmaMax, LMin, pMax, want_disp, is_descrete)  
...
%   (1) Assign cluster centres taking points with maximal frequencies
...
%     (5) compute average distance for each cluster and overall average distance
        ...
        distances = Mixed_Eucl_Quat_dist(X, Z);
        [delta, deltaOverall] = Isodata_distances_step5_Quaternion(distances, sampleLables, frequencies, clusterFrequencies, n, nClusters);
...
%     (7) for each cluster compute standard deviation in each direction
            ...
            for ii = 1:n
                ...
                if curCluster ~= 0 % some pixels are not marked!
                    ...
                    V(curCluster, 1:3) =  V(curCluster, 1:3) + curFrequency * (X(ii,1:3) - Z(curCluster,1:3)).^2;
                    V(curCluster, 4:6) =  V(curCluster, 4:6) + curFrequency * ((X(ii,4:6) - Z(curCluster,4:6))*alpha).^2;
...
%     (8) split big clusters
    ...
    %     (9) Compute pairvise interclass distances
            interClassDists = Mixed_Eucl_Quat_dist(Z, Z); % in fact matrix is diagonal!
            interClassDists = tril(interClassDists, - 1);
            % make it diagonal
...
%             [row, col] = find(interClassDists <= LMin);
%             if ~isempty(row)
%               
%                 ids = sub2ind(size(interClassDists), row, col);
%                 values = interClassDists(ids);
%                 [~, IX] = sort(values, 'ascend');
%                 IX = IX(1:pMax);
%                 row = row(IX);
%                 col = col(IX);
