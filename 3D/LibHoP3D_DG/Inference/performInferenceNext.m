...
function [] = performInferenceNext(list_depths, list_El, list_mask, lenF, sigma, sigmaKernelSize, dxKernel, ...
                nPrevClusters, nCurClusters, nClusters, X, partsOut, partEntropy, displ, ...
                ...
                dataSetNumber, layerID, cluster1Centres, fieldSize, cluster1Bounds, numSimilar, is_overwrite)
    ...
    [XX, ~, emptyIndicator]            = convertToSurfaceDescriptor(X, lenCombs, layerID, nClusters, n2Clusters, fileForVisualizationPrevLayer,  ...
                       displ, fieldCenter, cluster1Centres, downsamplingScheme, clusterCurDepths); 
    [partsOutXX, ~, emptyIndicatorPOX] = convertToSurfaceDescriptor(partsOut, nCurClusters, layerID, nClusters, n2Clusters, fileForVisualizationPrevLayer,  ...
                       displ, fieldCenter, cluster1Centres, downsamplingScheme, clusterCurDepths);  % this are a first layer descriptors  
...
%   distances = Isodata_distances(XX, partsOutXX, lenCombs, nCurClusters, false, false);
    ...
    disp('pre-computing of a table of parts similarities');
    ...
    for j = 1:lenCombs
        ...
        curDistances = distances(j,:);     % distances from the combination to vocabulary parts
%         curDistances(curDistances == 0) = 100;  % similarity to itself
        
        minD = min(curDistances);
        idx = find(curDistances == minD);  % idx - id of the closest vocabulary element
        
        % distances from this vocabulary parts to all combinations
        curCDistances = distances(:,idx);
        
        % take few compositions that are closest to this vocabulary element
        [smallestDists, smIDs] = defineSmallestDistances(curCDistances, numSimilar);
        meargeThresh = 2.0 * smallestDists(end);
