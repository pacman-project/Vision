...
if ~isempty(normal)
    ...
    isFlip = false;
    if size(vertex,1) ~= size(normal,1)
       normal = normal'; % Vladislav's
       isFlip = true;
    end
    ...
    hold on;
    if isFlip
        quiver3(vertex(sel,1),vertex(sel,2),vertex(sel,3),normal(sel,1),normal(sel,2),normal(sel,3),normal_scaling); % Vladislav's
    else
