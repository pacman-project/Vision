...
function [graphLevel] = extractEdges(graphLevel, firstLevelAdjNodes, level1Coords, options, currentLevelId)
    ...
    if ~isempty(graphLevel)
        [graphLevel] = createEdgesWithLabels(graphLevel, firstLevelAdjNodes, level1Coords, options, currentLevelId, edgeNoveltyThr);
    end
end