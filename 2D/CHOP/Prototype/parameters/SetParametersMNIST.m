...
function [ options ] = SetParametersMNIST( datasetName, options )
    ...
    options.realGaborFlag = true;  % Real gabor filters are used for feature extraction!
    options.gaborFilterThr = 0; % Min response threshold for convolved features, 
                                  % taken as the percentage of max response 
                                  % in each image.
    options.innerGaborFilterThr = []; % Min response threshold for inner contours.
...
%    options.gabor.sigma = 2.5;            % Gabor filter parameters
 %   options.gabor.sigma = 1.21;    
    options.gabor.sigma = 0.7;    
    options.gabor.theta = 0;
    options.gabor.lambda = 1;
    options.gabor.psi = 0;
    options.gabor.gamma = 0.3;
    ...
    options.distType = 'sift'; % Either 'modal', 'hog' or 'hu', or 'sift', 'context'.
