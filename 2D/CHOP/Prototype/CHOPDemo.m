%> Name: CHOPDemo
%>
%> Description: This function is the entry function of a live demo for object 
%> detection using learned categories in $datasetName$. After CHOP is
%> learned, a set of features from each training image is extracted, and a 
%> categorization model using SVM is learned with the features. In
%> detection, the candidate windows are located using higher level
%> realizations in test images, and each possible window is evaluated using a
%> number of pre-processing techniques and a classification code. If there
%> are renderings of sample objects from certain poses in the render/ folder,
%> they are overlaid with the original image and shown in a separate window.
%> In addition, activated features are also shown in a graph.
%>
...
%>
%> Updates
%> Ver 1.0 on 16.09.2014 Additional commentary and removal of global params
function [] = CHOPDemo(datasetName)
    %% Program initializations
    global state, global vocabCounts;
    ...
    if exist([pwd '/output/' datasetName '/vb.mat'], 'file')
        load([pwd '/output/' datasetName '/vb.mat'], 'vocabulary', 'redundantVocabulary', 'modes');
    ...
    while true
        ...
        if ~strcmp(state,'Exit') && isvalid(vid)
            ...
            if strcmp(state, 'Active')
                oldImg = img;
                [activationImg, detectionImg, partImg] = processImage(img, datasetName, ...
                    testFileName, vocabulary, redundantVocabulary, modes, options);
                state = 'Pending';
...
function [activationImg, detectionImg, partImg] = processImage(img, datasetName, ...
    testFileName, vocabulary, redundantVocabulary, modes, options)
    global vocabCounts;
    ...
    singleTestImage(testFileName, vocabulary, redundantVocabulary, modes, options);
